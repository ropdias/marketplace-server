...
describe('Count the number of views per day received by the seller in 30 days', () => {
  ...
  test('[GET] /sellers/metrics/views/days - should be able to count product views per day from last 30 days', async () => {
    ...
    // Create valid views: sellers 1 and 2 viewing products from seller in the last 30 days
    ...
    expect(response.body).toMatchObject({
      viewsPerDay: expectedArray,
    })
  })

  test('[GET] /sellers/metrics/views/days - should return 401 when no authentication cookie is provided', async () => {
  ...
  test('[GET] /sellers/metrics/views/days - should return 401 when invalid authentication cookie is provided', async () => {
    const response = await request(httpServer)
      .get('/sellers/metrics/views/days')
  ...
  test('[GET] /sellers/metrics/views/days - should return 404 if seller is not found', async () => {
    const { cookieString, seller } = await createSellerAndLoginWithCookie(
      sellerFactory,
      jwtService,
    )
